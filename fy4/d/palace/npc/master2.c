...
{ 
        if (ob->query("class")=="yinshi")
                ...
                message_vision(HIW"$N摇了摇头，说道：“一日无法胜过那个萧老儿，我怎么有心思收徒呢？”\n"NOR,this_object());
...
void do_recruit(object ob)
{
         ...
         {
                        if (ob->query_temp("despite_xiao"))
                        {
                                command("heihei "+ob->query("id"));
                                return;
                        }
                        message_vision(HIW"$N上上下下看了看$n，忽然哈哈大笑起来：“你不是箫老儿 的徒弟吗？ 
...
{
    if( (string) ob->query("class") == "yinshi" && ob->query("marks/master_lan"))
        return 1;
        return 0;
} 
