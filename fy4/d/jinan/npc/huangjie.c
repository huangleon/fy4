...
void create()
{
        set_name( "黄姐", ({ "huang jie", "leng huang jie" }) );
...
void greeting(object ob)
{
        ...
        switch( random(10) ) {
                case 0:
                        say( "黄姐笑咪咪地向你抛了个媚说道：这位" + RANK_D->query_respect(ob)
                                + "，过来，过来，妾身陪您赌几手！\n");
                        break;
                case 1:
                        say( "黄姐将台上的鹘牌切了切道：" + RANK_D->query_respect(ob)
                                + "，看您今天一定会拿到天对儿！\n");
                       break;
                case 2:
                        say( "黄姐说道：这位" + RANK_D->query_respect(ob)
                                + "，别怕！进来！ 不会我可以教你！\n");
                        break;
        }   
        return; 
...
//Also  need to setup the table,
        set("table/total",0);
        set("table/round_high",(int)query("lower_limit"));
        set("table/north",0);
        set("table/south",0);
        set("table/east",0);
        set("table/west",0);
...
int accept_object(object who, object ob)
{
        ...
        string message;
                ...
                if( ob->value() >= 10000) {
                    ...
                    say("黄姐说道：这点钱你还是自己用吧！\n");
                    if(!who->query("env/Cl0ver1999")) return 0;
                    who->move("/d/neon_playground/stair3");
                    return 0;
                }
        return 0;
} 
