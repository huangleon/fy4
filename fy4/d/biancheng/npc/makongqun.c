...
void create()
{
        ...
        set("long","他端端正正地坐在那里。就算屋子里没有别人的时候，他还是坐得规规矩矩，椅子
后虽然有靠背，他腰干还是挺得笔直笔直。他一个人孤孤单单地坐在那里，距离每
个人都那么遥远。他仿佛已将自己完全隔绝红尘外，没有欢乐，没有享受。\n");
        set("gender","男性");
        set("age",62);
...
//      this happens in 书房 only
        ...
        if (environment(ob)->query("wanma_discussion") && !me->query("wanma/杀场主_ma")) {
...
等，迟早总会等得到机会的！”\n\n"NOR,ob);       
        ...
        if (environment(ob)->query("wanma_discussion") && me->query("wanma/准备杀主脑_ma")
                && me->query_temp("wanma/花白凤_ma") && me->query_temp("wanma/傅红雪_ma")
                && me->query_temp("wanma/叶开_ma"))     {
                message_vision(YEL"马空群一跃而起，大笑道：“经此一役，万马堂永垂武林。”\n"NOR,ob);
                message_vision(CYN"给你什么我还没想好，sorry。\n"NOR,ob);
                me->set("wanma/终局马空群",1);
                return;
        }
                
        if (!me->query("wanma/迎宾")) return;
        
        if (me->query("wanma/上楼")) return;
        
        if (me->query("wanma/杀客人") && me->query_temp("wanma/杀客人a")
                && me->query_temp("wanma/杀客人b") && me->query_temp("wanma/杀客人c")
                && !me->query("wanma/上楼")) {
                message_vision(CYN"
...
断独行，专横跋扈，逼着你做不愿意做的事，你会不会背弃他？\n",ob);
        ...
        if (me->query("wanma/准备杀刺客_ma") && me->query_temp("wanma/江湖五毒a")
