...
void greeting(object me)
{
        ...
        if (me->query("wanma/寻地洞") && !me->query("wanma/杀场主_bai")) {
                call_out("event1",1,me,0);      
        }
        if (me->query("wanma/杀场主_bai")
                && me->query_temp("wanma/杀花满天_bai") && me->query_temp("wanma/杀云在天_bai")
                && me->query_temp("wanma/杀公孙断_bai") && me->query_temp("wanma/杀马芳玲_bai")
                && me->query_temp("wanma/杀小虎子_bai") 
                && !me->query("wanma/准备杀同伙_bai")) {        
                message_vision(CYN"
...
大经，你要找出其余的三个，杀光他们的一家老少。”\n"NOR,ob);
                me->set("wanma/准备杀同伙_bai",1);
        }
        if (me->query("wanma/准备杀同伙_bai") 
                && me->query_temp("wanma/易大经_bai") && me->query_temp("wanma/薛斌_bai")
                && me->query_temp("wanma/桃花娘子_bai") && me->query_temp("wanma/柳东来_bai")
                && me->query_temp("wanma/郭威_bai") 
                && !me->query("wanma/准备杀主脑_bai")) {
                message_vision(CYN"
花白风转过身去，跪在黑色的神龛前，喃喃低语，没有人能听到她在说什么，
片刻，。。\n"NOR,ob);
                me->set("wanma/准备杀主脑_bai",1);
        }       
        if (me->query("wanma/准备杀主脑_bai") && me->query_temp("wanma/杀马空群")
                && me->query_temp("wanma/杀丁白云") && me->query("wanma/结局_bai",1)){
                message_vision("hmm, what's the reward here?\n",ob);
                me->set("wanma/结局_bai",1);
        }
        return;
} 
void die()
{
        ...
        if(me->query("wanma/花白风_ma"))
                        me->set_temp("wanma/花白风_ma",1);
        ::die();
}  
string *event1_msg=     ({
        CYN"
