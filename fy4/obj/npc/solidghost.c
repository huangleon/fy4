...
void invocation(object who, int level)
{
        ...
        set("cor",10+random(level/8)); 
        set_skill("cursism",level/4);
        set_skill("celecurse",level/4);
        
   owner_exp=who->query("combat_exp");
        owner_hp= who->query("max_kee");
        
        map_skill("cursism","celecurse"); 
        set("max_kee",500+random(level*level/60)+owner_hp/10);
        ...
        set("combat_exp", 100000 + level*level*5+random(level*level*level/40)+owner_exp/5);
        set("possessed",who); 
