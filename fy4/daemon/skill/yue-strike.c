...
{       int busy_time,alr_busy_time;
        ...
        if( !me->query("m_success/结拜郭大路")&& userp(me))                                     
                return 0;
        skill=me->query_skill("yue-strike",1);
        
        if (userp(me) && !random(4-skill/60) && 4*random(my_exp)>your_exp) {
                me->set_temp("counter_action",1);
                me->set_temp("counter_msg",HIY"$N利用$n出招后的漏洞乘机反击。\n"NOR);
        }
